...
from blowfish_sys import Blowfish
...
class PasswordManager:
    ...
    def welcome_new_user(self):
        ...
        label1 = Label(self.window, text="Create New Admin Password")
    ...
    def login_user(self):
        ...
        label1 = Label(self.window, text="Enter Your Admin Password")
        label1.config(anchor=CENTER)
    ...
    def save_master_password(self, eb1, eb2):
        ...
        if password1 == password2:
            password_object = Blowfish()
            hashed_password = self.encrypt_password(password_object, password1)[0]
    ...
    def check_master_password(self, eb):
        input_password = Blowfish()
        hashed_password = self.encrypt_password(input_password, eb.get())[0]
    ...
    def password_vault_screen(self):
        ...
        if self.cursor.fetchall():
            ...
            while True:
                ...
                password_label = Label(second_frame, text = Blowfish().blowfish_encrypt(array[i][3])[0])
    ...
    def encrypt_password(self, password_object, password):
        encoded_text = password_object.blowfish_encrypt(password)
        return encoded_text
